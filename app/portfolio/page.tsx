...
const Page = () => {
 ...
 return (
  <div className='xl:ml-[var(--left-sidebar-width)]  pb-[calc(var(--bottom-navbar-height)+1rem)]'>
   <div className="flex">
    {/* LEFT SECTION  */}
    <section className="flex-1 pe-4 md:pe-10 lg:pe-0">
     {/* projects */}
     {projects.map((project) => (
      <Link
       href={`/portfolio/${project.id}`}
       key={project.id}
       className="px-4 py-7 lg:px-6 lg:py-9 border border-primary border-s-0 border-t-0 rounded-full rounded-s-none flex flex-col gap-1 nth-child group relative overflow-hidden before:content-[''] before:absolute before:inset-0 before:bg-primary before:scale-x-0   before:origin-left hover:before:scale-x-100 before:transition-transform before:duration-600 "
      ...
      >
       ...
       <h4 className="font-display font-bold text-2xl md:text-3xl lg:text-3xl xl:text-4xl group-hover:text-white relative z-10">{project.title}</h4>
      </Link>
     ))}
    </section>
    {/* RIGHT SECTION */}
